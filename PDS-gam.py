...
GAM_array=np.array([0.01, 0.07, 0.5, 1, 5])
...
np.savetxt('PSNR-gam-optnoise.txt', PSNR_noise)
np.savetxt('SSIM-gam-optnoise.txt', SSIM_noise)

np.savetxt('PSNR-gam.txt', PSNR)
np.savetxt('SSIM-gam.txt', SSIM)
...
for k in range(GAM_num):
  for n in range(img_num):
    ...
    filename_est=f'{n}(est){k}-gam1.png'
