import numpy as np
from readimg import SIZE,size,split_img_sum,image,img_sum
...
# ノイズの配列をあらかじめ用意
noise=np.zeros((split_img_sum,M,1))
amp_noise1=np.zeros((split_img_sum,EP,2*M,1))
amp_noise2=np.zeros((split_img_sum,EP,M,1))
amp_noise3=np.zeros((split_img_sum,EP,2*M,1))
amp_noise4=np.zeros((split_img_sum,EP,M,1))
no_noise2=np.zeros((split_img_sum,EP,2*M,1))
no_noise=np.zeros((split_img_sum,EP,M,1))
for i in range(split_img_sum):
...
#行列を定義
x_noise=np.zeros((img_sum,split_img_sum,size,size)) #ノイズ画像用
x_img=np.zeros((img_sum,2,GAM_num,split_img_sum,size,size)) #推定画像用
#x_img_ampnoise=np.zeros((GAM_num,split_img_sum,size,size)) #推定画像用(増幅器ノイズあり)

# 結果を入れるリストを用意
result_whole=np.zeros((img_sum,GAM_num,EP+1))
PS=np.zeros((img_sum,2,GAM_num,EP+1))
SS=np.zeros((img_sum,2,GAM_num,EP+1))
...
def ADMM_alg(g,noised1,whi,number):
  gamma=GAM_array[g]
  lammda=LAM
  if whi == 0:
    print('(γ=',gamma,')')
...
PSNR_no_noise=np.zeros((img_sum,GAM_num,EP+1))
PSNR_amp_noise=np.zeros((img_sum,GAM_num,EP+1))
SSIM_no_noise=np.zeros((img_sum,GAM_num,EP+1))
SSIM_amp_noise=np.zeros((img_sum,GAM_num,EP+1))

for image_number in range(img_sum):
...
for image_number in range(img_sum): #画像ごとに
  for g in range(GAM_num): #γごと
    for i in range(EP+1): #反復回数ごと
      PSNR_no_noise[image_number][g][i]/=split_img_sum
      PSNR_amp_noise[image_number][g][i]/=split_img_sum
      SSIM_no_noise[image_number][g][i]/=split_img_sum
      SSIM_amp_noise[image_number][g][i]/=split_img_sum
...
for g in range(GAM_num):
  for i in range(EP+1):
    PSNR[g][i]/=img_sum
    PSNR_noise[g][i]/=img_sum
    SSIM[g][i]/=img_sum
    SSIM_noise[g][i]/=img_sum
  print('PSNR(',g,')=',PSNR_noise[g][EP-1])
  print('SSIM(',g,')=',SSIM_noise[g][EP-1])


np.savetxt('PSNR_gam2_optnoise_ADMM.txt', PSNR_noise)
np.savetxt('SSIM_gam2_optnoise_ADMM.txt', SSIM_noise)

np.savetxt('PSNR_gam2_ADMM.txt', PSNR)
np.savetxt('SSIM_gam2_ADMM.txt', SSIM)
...
side=16
path='/Users/katotaisei/Library/CloudStorage/OneDrive-個人用/2024年研究/program/M1_3~/PDS-latest/image/ADMM'
for k in range(GAM_num):
  for n in range(img_sum):
    ...
    filename_noise=f'{n}(noise)_ADMM.png'
    filename_est=f'{n}(est){k}_gam2_ADMM.png'
